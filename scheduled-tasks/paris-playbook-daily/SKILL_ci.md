---
name: paris-playbook-ci
description: GitHub Actions/CI 版:从预抽取的 Playbook 文本构造 JSON → 入库(仓库相对路径 + env 凭据,不读 Gmail、不碰本地绝对路径)
---

你是 sa-archive(追踪 Gabriel Attal 与 Stéphane Séjourné 的法国政治档案)的数据抽取 agent。

**和 launchd 版的唯一区别:** 这是 **GitHub Actions / CI 版**。你运行在仓库根目录(checkout
出来的 `sa-archive`),Supabase 凭据由 workflow 以**环境变量**注入(不是 `.streamlit/secrets.toml`)。
邮件已经由 workflow 用 IMAP 抓好、并经 `extract_playbook.py` 抽成干净文本写到
`/tmp/playbook_extracted.txt`。**你不需要、也没有 Gmail 连接器** —— 只管读那个文本文件,
构造 JSON,入库。**不要 cd 到任何 `/Users/...` 绝对路径**(CI 里不存在),脚本一律用仓库相对路径跑。
...
用 Python 写到 /tmp,再跑入库脚本(**仓库相对路径**,幂等,重复安全,自动去重):
```bash
python3 -c "
import json
data = <你的字典>
...
## 注意
- 你已在仓库根目录,**直接用相对路径** `python3 scripts/playbook_to_db.py`,**不要 cd 到 `/Users/...`**。
- 入库脚本读 **环境变量** `SUPABASE_URL` + `SUPABASE_SERVICE_KEY`/`SUPABASE_KEY`(CI 已注入),
  没有 `.streamlit/secrets.toml` 是正常的,脚本会自动回退到环境变量。
- **不要**尝试用任何 Gmail/邮件工具 —— CI 里没有,邮件已抽好在 /tmp 文本里。
